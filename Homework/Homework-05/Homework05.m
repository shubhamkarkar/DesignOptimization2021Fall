...
% (marked by ***). Note that you need to do so for every involv
...
% Here, we indicate the objective function by assigning a % variable
% to the function handle, for example:
...
% Checking the feasibility of the first point.
if max(g(x0)>0)
...
%The active set strategy is used to solve the QP subproblem in the following code.
